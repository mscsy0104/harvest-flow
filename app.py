...
from harvest_flow.adapters.metadata_db import SqliteMetadataAdapter
from harvest_flow.adapters.ssg import GitHubSSGAdapter
from harvest_flow.adapters.vector_db import QdrantVectorAdapter
from harvest_flow.agent_pipeline import generate_ai_metadata
...
from harvest_flow.logger import logger
from harvest_flow.parser import parse_markdown
...
vector_db_adapter: VectorDB = QdrantVectorAdapter()
metadata_db_adapter: MetadataDB = SqliteMetadataAdapter()
ssg_publisher: SSGPublisher = GitHubSSGAdapter()
...
def pipeline_trigger(file_path: str) -> bool:
    ...
    processed_content = build_processed_content(
        yaml_text=yaml_text,
        body_text=body_text,
    ...
    metadata_db_adapter.save_note_metadata(filename, mtime, "출간")
    vector_payload = {
        "title": filename,
        "status": "출간",
    ...
    vector_db_adapter.upsert_document(filename, body_text, vector_payload)
    logger.info(
    ...
    published = ssg_publisher.publish_content(filename)
    if not published:
        logger.error("출하 실패(publish client=%s): %s", PUBLISH_CLIENT, filename)
        return False
    return True
...
def cleanup_note_artifacts(file_path: str) -> None:
    ...
    vector_db_adapter.delete_document(filename)
...
def handle_soft_delete(file_path: str) -> bool:
    ...
    filename = path.name
    if not ssg_publisher.unpublish_content(filename):
        mark_delete_failed(
            filename,
...
def handle_second_review_publish(file_path: str) -> bool:
    ...
    if not processed:
        logger.warning("출간 파이프라인 실패 또는 스킵: %s", path.name)
        return False
...
def process_due_publications() -> None:
    ...
    for row in due_rows:
        ...
        if not path.exists():
            continue
        try:
            published = handle_second_review_publish(str(path))
            if published:
                continue
            # 실패 결과를 명시적으로 반환한 경우도 지연 재시도합니다.
            retry_at = _now_ts() + 60
...
def process_due_deletions() -> None:
    ...
    for row in due_rows:
        ...
        try:
            deleted = handle_soft_delete(str(path))
            if deleted:
                continue
            retry_at = _now_ts() + 60
            reschedule_delete_due_at(
            ...
            logger.info("지연 삭제 재시도 예약: %s (due in 1 min)", filename)
...
def main() -> None:
    ...
    try:
        ...
        from harvest_flow.database import get_vector_db_client
