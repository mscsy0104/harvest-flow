...
### (선택) Docker Compose 한 번에 실행

```bash
cp .env.example .env
# (개인 Vault를 쓰려면) cp docker-compose.override.example.yml docker-compose.override.yml 후 경로 수정
docker compose up -d
```
...
- OSS 기본값: `notes_vault`, `publish_content` (샘플 데이터는 `samples/` 사용)
- 개인 운영: `templates/harvest-flow-my`의 `.env.example`/override로 개인 Vault + Quartz 경로 연결

### 핵심 기능

- **Vector Search**: Qdrant 기반 임베딩 색인/검색
- **RAG Assistant**: 노트 근거 기반 질의응답
- **SSG Publishing**: Quartz/GitHub Pages 출간 자동화
...
| `APP_CACHE_SIM_THRESHOLD` | `0.93` | 시맨틱 캐시 적중 임계값(0~1) |
| `APP_CACHE_RECENT_LIMIT` | `20` | 최근 질의 캐시 목록 기본 조회 개수 |
| `GIT_PUSH` | `0` | 실제 Git push 수행 여부 (기본 OFF 권장) |
