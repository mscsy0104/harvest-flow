...
| **경로 구조** | `prototype/` 기준 `Path(__file__).resolve()/obsidian_vault`, `Path(__file__).resolve()/quartz_content`, `Path(__file__).resolve()/qdrant_local` 일치. |
...
| 최종 승인 → BLOG_CONTENT_DIR | 없음 | `status: 출간`이면 **바로** BLOG_CONTENT_DIR 복사·(시도) git push. 대시보드 승인 게이트 없음. |
