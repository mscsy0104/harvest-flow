...
def get_sqlite_connection() -> sqlite3.Connection:
    ...
    return sqlite3.connect(METADATA_DB)
...
def reschedule_delete_due_at(
    filename: str,
    *,
    due_at: float,
    transition_ts: float | None = None,
) -> None:
    """삭제 재시도를 위해 due_at만 지연 갱신합니다."""
    conn = get_metadata_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            ...
            UPDATE {METADATA_DB_TABLE}
            SET delete_due_at = ?,
                delete_status = COALESCE(delete_status, 'requested'),
                last_transition_at = COALESCE(?, last_transition_at)
            WHERE filename = ?
            """,
            (due_at, transition_ts, filename),
        )
        conn.commit()
    finally:
        conn.close()
