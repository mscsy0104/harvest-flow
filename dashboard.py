...
from harvest_flow.agent_rag import iter_rag_answer
from harvest_flow.config import (
    NOTES_APP_NAME,
    NOTES_DIR,
    PIPELINE_MODEL,
    PUBLISH_CLIENT,
...
from harvest_flow.logger import logger, read_recent_logs
from harvest_flow.semantic_cache import (
    get_cached_answer,
    list_recent_cached_queries,
    save_to_cache,
)
from harvest_flow.utils import (
    format_ts,
    generate_compact_id,
    get_file_created_ts,
    parse_iso_ts,
)
from harvest_flow.workflow import (
...
def load_or_fallback(label: str, loader, fallback):
    """대시보드 데이터 로더를 감싸 장애를 노출 가능하게 만듭니다."""
    try:
        return loader(), None
    except Exception as exc:
        logger.exception("대시보드 데이터 로드 실패[%s]: %s", label, exc)
        return fallback, f"{label} 로드 실패: {exc}"
...
def safe_html_text(value: object) -> str:
    """unsafe_allow_html 렌더링 시 텍스트를 이스케이프합니다."""
    if value is None:
        return "-"
    text = str(value)
...
obsidian_cnt, issue = load_or_fallback("초안 문서 수", get_obsidian_count, 0)
if issue:
    runtime_issues.append(issue)
publish_cnt, issue = load_or_fallback("출하 문서 수", get_publish_count, 0)
if issue:
    runtime_issues.append(issue)
notes_app_name = NOTES_APP_NAME.title()
vector_db_name = VECTOR_DB_CLIENT.title()
publish_name = PUBLISH_CLIENT.title()
vector_data, issue = load_or_fallback("벡터 저장소 스냅샷", get_vector_data, (0, []))
if issue:
    runtime_issues.append(issue)
vector_cnt, vector_points = vector_data
...
workflow_counts, issue = load_or_fallback("워크플로우 단계 집계", get_workflow_counts, {})
if issue:
    runtime_issues.append(issue)
if runtime_issues:
    st.warning("일부 대시보드 데이터 로드에 실패했습니다. 아래 항목을 확인해 주세요.")
    for issue_text in runtime_issues:
        st.caption(f"- {issue_text}")
        ...
        if not rows:
            ...
            with right:
                ...
                if not selected_row:
                    ...
                    selected_id = safe_html_text(selected_row.get("id", "-"))
                    selected_name = safe_html_text(selected_row.get("name", "-"))
                    selected_created_at = safe_html_text(selected_row.get("created_at", "-"))
                    selected_published_at = safe_html_text(selected_row.get("published_at", "-"))
                    st.markdown(
                        f"""
                        <div class="publish-info-card">
                            <div class="publish-info-row">
                                <span class="publish-info-key">id</span>
                                <span class="publish-info-value">{selected_id}</span>
                            </div>
                            <div class="publish-info-row">
                                <span class="publish-info-key">filename</span>
                                <span class="publish-info-value">{selected_name}</span>
                            </div>
                            <div class="publish-info-row">
                                <span class="publish-info-key">파일 생성일</span>
                                <span class="publish-info-value">{selected_created_at}</span>
                            </div>
                            <div class="publish-info-row">
                                <span class="publish-info-key">블로그 출간일</span>
                                <span class="publish-info-value">{selected_published_at}</span>
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True,
...
# 5. 🗃️ 삭제 이력 (soft delete)
    ...
    rows, deleted_issue = load_or_fallback("삭제 이력 조회", get_deleted_metadata_rows, [])
    if deleted_issue:
        st.error(deleted_issue)
    if not rows:
